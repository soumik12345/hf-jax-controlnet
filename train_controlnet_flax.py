...
def log_validation(pipeline, pipeline_params, controlnet_params, tokenizer, args, rng, weight_dtype, step, wandb_table):
    ...
    if args.report_to == "wandb":
        ...
        for log in image_logs:
            ...
            generated_images_row = []
            formatted_images.append(wandb.Image(validation_image, caption="Controlnet conditioning"))
            for image in images:
                image = wandb.Image(image, caption=validation_prompt)
                generated_images_row.append(wandb.Image(image))
                formatted_images.append(image)

        wandb_table.add_data(step, validation_prompt, wandb.Image(validation_image), generated_images_row)
        wandb.log({"validation": formatted_images})
        return image_logs, wandb_table
...
def main():
    ...
    if jax.process_index() == 0 and args.report_to == "wandb":
        ...
        wandb_table = wandb.Table(columns=["Step", "Prompt", "Conditioning-Image", "Generated-Images"])
    ...
    for epoch in epochs:
        ...
        for batch in train_dataloader:
            ...
            if global_step % args.checkpointing_steps == 0 and jax.process_index() == 0:
                ...
                artifact.add_dir(f"{args.output_dir}/{global_step}")
                wandb.log_artifact(artifact, aliases=[f"step-{global_step}", "latest"])
    ...
    if jax.process_index() == 0:
        ...
        wandb.log({"Validation": wandb_table})
